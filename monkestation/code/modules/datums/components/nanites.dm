...
/datum/component/nanites/Initialize(amount = 100, cloud = 0)
	...
	if(isliving(parent))
		...
		if(!(host_mob.mob_biotypes & (MOB_ORGANIC|MOB_UNDEAD|MOB_ROBOTIC)) || istype(host_mob, /mob/living/silicon)) //Shouldn't happen, but this avoids HUD runtimes in case a silicon gets them somehow.
			return COMPONENT_INCOMPATIBLE
...
/datum/component/nanites/proc/reject_excess_nanites()
	...
	switch(excess)
		...
		if((NANITE_EXCESS_BURST + 0.1) to INFINITY) //Way too many nanites, they just leave through the closest exit before they harm/poison the host
			...
			host_mob.adjust_temp_blindness(1.5 SECONDS) //nanites coming out of your eyes
...
/datum/component/nanites/proc/check_viable_biotype()
	...
	if(!(host_mob.mob_biotypes & (MOB_ORGANIC|MOB_UNDEAD|MOB_ROBOTIC)) || istype(host_mob, /mob/living/silicon))

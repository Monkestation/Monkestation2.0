...
/datum/preference/choiced/ornithid_wings/icon_for(value)
	...
	if(arm_wings.icon_state == null || arm_wings.icon_state == "none")
		var/icon/invalid_icon = icon('icons/mob/landmarks.dmi', "x")
		return invalid_icon
	var/icon/final_icon = icon(arm_wings.icon, "m_arm_wings_[arm_wings.icon_state]_FRONT")
	return final_icon
...
/datum/preference/choiced/tail_avian/icon_for(value)
	...
	if(tail_avian.icon_state == null || tail_avian.icon_state == "none")
		var/icon/invalid_icon = icon('icons/mob/landmarks.dmi', "x")
		return invalid_icon
...
/datum/preference/choiced/plumage/icon_for(value)
	...
	if(ears_avian.icon_state == null || ears_avian.icon_state == "none")
		var/icon/invalid_icon = icon('icons/mob/landmarks.dmi', "x")
		return invalid_icon
	var/icon/final_icon = icon(ears_avian.icon, "m_ears_avian_[ears_avian.icon_state]_FRONT")
	return final_icon
...
/datum/preference/choiced/ornithid_underwear
	savefile_key = "ornithid underwear"
	savefile_identifier = PREFERENCE_CHARACTER
	main_feature_name = "Ornithid Underwear"
	category = PREFERENCE_CATEGORY_CLOTHING
	should_generate_icons = TRUE

/datum/preference/choiced/ornithid_underwear/init_possible_values()
	return assoc_to_keys_features(GLOB.ornithid_underwear_list)

/datum/preference/choiced/ornithid_underwear/icon_for(value)
	...
	if (isnull(lower_half))
		...
		lower_half.Blend(icon('monkestation/code/modules/the_bird_inside_of_me/icons/ornithid_parts_greyscale.dmi', "ornithid_r_leg"), ICON_OVERLAY) //TODO
		lower_half.Blend(icon('monkestation/code/modules/the_bird_inside_of_me/icons/ornithid_parts_greyscale.dmi', "ornithid_l_leg"), ICON_OVERLAY) //TODO

	return generate_underwear_icon(GLOB.ornithid_underwear_list[value], lower_half, COLOR_ALMOST_BLACK) //TODO

/datum/preference/choiced/ornithid_underwear/apply_to_human(mob/living/carbon/human/target, value)
	target.ornithid_underwear = value

/datum/preference/choiced/ornithid_underwear/is_accessible(datum/preferences/preferences)
	if (!..(preferences))
		return FALSE

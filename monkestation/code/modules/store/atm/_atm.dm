...
/obj/machinery/atm/attacked_by(obj/item/attacking_item, mob/living/user)
	. = ..()
	// Give someone a chance to steal the item >:]
	if(!do_after(user, 1 SECONDS, src))
		to_chat(user, "You decide not to deposit \the [attacking_item].")
		return

	// Make sure they still have the item
	if(!QDELETED(attacking_item) && !user.temporarilyRemoveItemFromInventory(attacking_item))
		return
	...
	else if(attacking_item in subtypesof(/obj/item/stack/spacecash))

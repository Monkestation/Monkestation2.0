...
/datum/surgery/robot_tox_clean/can_start(mob/user, mob/living/carbon/target)
	if((target.dna.species.reagent_tag & PROCESS_SYNTHETIC) && target.stat == DEAD) // This surgery is only available if you can't process most Toxin-clearing chems and you're not alive to process system cleaner.
		return TRUE
	return FALSE

...
/obj/item/lighter
	...
	var/lit = FALSE
	/// Wheter the lighter has a lid.
	var/has_lid = TRUE
	/// Wheter the lid is open.
	var/lid_open = FALSE
	...
	var/maximum_fuel = 10
	///The amount of fuel burnt each process. Set 0 to not use fuel.
	var/fuel_usage_rate = 0.1
...
/obj/item/lighter/Initialize(mapload)
	. = ..()
	create_reagents(maximum_fuel, REFILLABLE | DRAWABLE)
	reagents.add_reagent(fuel_type, maximum_fuel)
	if(!overlay_state)
...
/obj/item/lighter/proc/create_lighter_overlay()
	if(has_lid)
		return mutable_appearance(icon, "lighter_overlay_[overlay_state][lid_open ? "-on" : ""]")
...
/obj/item/lighter/proc/set_lit(new_lit, snuffed = FALSE)
	if(lit == new_lit)
		return
	...
	if(lit)
		...
		if(!snuffed)
			...
			playsound(src.loc, 'sound/items/lighter/cig_snuff.ogg', 50, 1, -2)
...
/obj/item/lighter/attack_self(mob/living/user)
	...
	var/epic_fail = FALSE
	if(fuel_usage_rate && get_fuel() <= fuel_usage_rate)
		if(fancy)
			user.visible_message(
				span_notice("Without even breaking stride, [user] flips open and tries to light [src] in one smooth movement. Would be cool if it actually lit though."),
				span_notice("Without even breaking stride, you flip open [src] in one smooth movement but it fails to light.")
			)
			playsound(src.loc , 'sound/items/lighter/zippo_on.ogg', 100, 1)
		else
			user.visible_message(
				span_notice("After a few attempts, [user] fails to light [src]."),
				span_notice("After a few attempts, you fail to light [src].")
			)
			playsound(src.loc, 'sound/items/lighter/lighter_on.ogg', 100, 1)
		epic_fail = TRUE
...
/obj/item/lighter/proc/toggle_lid(lid_state)
	lid_open = lid_state
...
/obj/item/lighter/use(used = 0)
	...
	reagents.remove_reagent(fuel_type, used)
...
/obj/item/lighter/slime
	...
	overlay_state = "slime"
	maximum_fuel = 15
	fuel_type = /datum/reagent/medicine/pyroxadone
...
/obj/item/lighter/commemorative
	name = "commemorative zippo"
	desc = "A commemorative limited edition zippo lighter created for Nanotrasen's corporate events to commemorate the launch of Space Station 13."
	overlay_state = "captain"
	icon_state = "captain"
	maximum_fuel = 15
...
/obj/item/lighter/mime
	...
	overlay_state = "mime"
	fuel_type = /datum/reagent/consumable/nothing
	fuel_usage_rate = 0
	grind_results = list(/datum/reagent/iron = 1, /datum/reagent/toxin/mutetoxin = 5)
	light_outer_range = 0
	light_power = 0
...
/obj/item/lighter/bright
	...
	fuel_type = /datum/reagent/flash_powder
	grind_results = list(/datum/reagent/iron = 1)
	light_outer_range = 8

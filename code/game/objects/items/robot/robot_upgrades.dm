...
/obj/item/borg/upgrade/surgery
	...
	var/list/adv_surgical_tools = list(obj/item/circular_saw/augment, item/scalpel/borg,obj/item/cautery/augment, obj/item/retractor/augment, obj/item/hemostat/augment/)
	var/list/surgical_tools = list(obj/item/circular_saw/circular, item/scalpel/scalp, obj/item/cautery/cauter, obj/item/retractor/retra, obj/item/hemostat)

/obj/item/borg/upgrade/surgery/action(mob/living/silicon/robot/R, user = usr)
	. = ..()
	if(.)
		for(var/surgtool in surgical_tools)
			for(surgtool in R.model.modules)
				R.model.remove_module(surgtool, TRUE)

...
/obj/item/sharpener/attackby(obj/item/I, mob/user, params)
	...
	if(I.force >= max || I.throwforce >= max) //So the whetstone never reduces force or throw_force
		...
		return
	if(requires_sharpness && !I.get_sharpness() && !(is_type_in_list(I, list(/obj/item/reagent_containers/cooking_container/pan, /obj/item/melee/flyswatter)))) //monkestation edit for the last part. RIP Ben

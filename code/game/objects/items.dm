...
/obj/item/examine(mob/user) //This might be spammy. Remove?
	...
	if((resistance_flags & INDESTRUCTIBLE) && !(resistance_flags & SHOW_RESISTANCE))
		. += "[src] seems extremely robust! It'll probably withstand anything that could happen to it!"
	else
		if((resistance_flags & LAVA_PROOF) && !(resistance_flags & SHOW_RESISTANCE))
			. += "[src] is made of an extremely heat-resistant material, it'd probably be able to withstand lava!"
		if((resistance_flags & (ACID_PROOF | UNACIDABLE)) && !(resistance_flags & SHOW_RESISTANCE))
			. += "[src] looks pretty robust! It'd probably be able to withstand acid!"
		if((resistance_flags & FREEZE_PROOF) && !(resistance_flags & SHOW_RESISTANCE))
			. += "[src] is made of cold-resistant materials."
		if((resistance_flags & FIRE_PROOF) && !(resistance_flags & SHOW_RESISTANCE))

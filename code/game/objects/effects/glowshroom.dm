...
/obj/structure/glowshroom/Initialize(mapload, obj/item/seeds/newseed, icon_variant)
	. = ..()
	if(!blacklisted_glowshroom_turfs)
		blacklisted_glowshroom_turfs = typecacheof(list(/turf/open/lava, /turf/open/water, /turf/open/floor/plating))
	if(icon_variant)
		src.icon_variant = icon_variant
	if(istype(newseed))
		myseed = newseed

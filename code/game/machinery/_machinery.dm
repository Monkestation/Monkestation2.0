...
/obj/machinery/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	...
	update_last_used(user)
	if(isAI(user) && !SScameras.turf_visible_by_cameras(get_turf(src), user)) //We check if they're an AI specifically here, so borgs/adminghosts/human wand can still access off-camera stuff.

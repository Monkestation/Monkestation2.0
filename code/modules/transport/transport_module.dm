...
/obj/structure/transport/linear/LateInitialize(mapload_arg)
	. = ..()
		...
		for(var/turf/dest_turf as anything in entering_locs)
			...
			for(var/mob/living/victim_living in dest_turf.contents)
				...
				if(transport_controller_datum.ignored_smashthroughs[victim_living.type])
					continue
				var/should_increment_count = ismob(victim_living) && victim_living.client // check this BEFORE we do any damage
				...
				if(should_increment_count && istype(transport_controller_datum, /datum/transport_controller/linear/tram))

...
/obj/machinery/microwave
	...
	/// Can this microwave be picked up?
...
/obj/machinery/microwave/update_overlays()
	...
	for(var/atom/movable/ingredient as anything in ingredients)
		var/mob/living/living_hotdog = ingredient
		var/lying_angle
		if(istype(living_hotdog))
			lying_angle = living_hotdog.get_lying_angle()
			living_hotdog.set_lying_angle(90)
		var/image/ingredient_overlay = image(ingredient, src)
		if(istype(living_hotdog))
			lying_angle = living_hotdog.get_lying_angle()
			living_hotdog.set_lying_angle(lying_angle)
...
/obj/machinery/microwave/mouse_drop_receive(obj/item/tool, mob/user, params)
	if(isliving(tool))
		var/mob/living/soon_to_be_hotdog = tool
		if(soon_to_be_hotdog.buckled || soon_to_be_hotdog.has_buckled_mobs())
			return
		stuff_mob_in(soon_to_be_hotdog, user)
		return

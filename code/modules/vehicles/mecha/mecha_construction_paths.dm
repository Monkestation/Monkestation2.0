...
//Maintenance Battle Tank
/datum/component/construction/unordered/mecha_chassis/mbt
	result = /datum/component/construction/mecha/mbt
	steps = list(
...
/datum/component/construction/mecha/mbt
	result = /obj/vehicle/sealed/mecha/maintenance_battle_tank
	base_icon = "mbt"

/datum/component/construction/mecha/mbt/get_frame_steps()
	return list(
		list(
			"key" = TOOL_CROWBAR,
		...
		list(
			"key" = TOOL_WRENCH,
			"back_key" = TOOL_CROWBAR,
			"desc" = "The tracks are installed, and the hydraulic systems can be connected with a <b>wrench</b>.",
			"forward_message" = "connected the hydraulic systems",
			"backward_message" = "removed tracks systems"
		),
		list(
			"key" = TOOL_SCREWDRIVER,
			"back_key" = TOOL_WRENCH,
			"desc" = "The tracks are completed, the engine can now be attached with a <b>screwdriver</b>.",
			"forward_message" = "connected engine",
			"backward_message" = "disconnected hydraulic systems"
		),
		list(
			"key" = /obj/item/stack/cable_coil,
			"amount" = 30,
			"back_key" = TOOL_SCREWDRIVER,
			"desc" = "you forgot more screws, you can secure the engine with <b>30 cable</b>.",
			"forward_message" = "ziptied engine",
			"backward_message" = "unscrewed engine"
		),
		list(
			"key" = TOOL_CROWBAR,
			"back_key" = TOOL_WIRECUTTER,
			"desc" = "The Engine 'should' stay in place, you can now lift the turret into place with a <b>crowbar</b>.",
			"forward_message" = "adjusted turret",
			"backward_message" = "removed zipties"
		),
		list(
			"key" = TOOL_WRENCH,
			"back_key" = TOOL_CROWBAR,
			"desc" = "You align the turret with the rest of the chassis, you can secure it with a <b>wrench</b>.",
			"forward_message" = "bolted turret",
			"backward_message" = "displaced turret"
		),
		list(
			"key" = TOOL_SCREWDRIVER,
			"back_key" = TOOL_CROWBAR,
			"desc" = "Now that the turret is secured, the autoloader can be alligned with a <b>screwdriver</b>.",
			"forward_message" = "alligned autoloader",
			"backward_message" = "broke bolts"
		),
		list(
			"key" = /obj/item/stack/cable_coil,
			"amount" = 5,
			"back_key" = TOOL_CROWBAR,
			"desc" = "The autoloader is ready to insert shells, if it could be powered, use some <b>cable</b> to connect the loader.",
			"forward_message" = "powered autoloader",
			"backward_message" = "misaligned autoloader"
		),
		list(
			"key" = TOOL_CROWBAR,
			"back_key" = TOOL_WIRECUTTER,
			"desc" = "now that the internals of the turret are ready, the gun should be inserted with <b>crowbar</b>.",
			"forward_message" = "inserted cannon",
			"backward_message" = "snipped cables"
		),
		list(
			"key" = TOOL_WRENCH,
			"back_key" = TOOL_CROWBAR,
...
/datum/component/construction/mecha/mbt/get_circuit_steps()
	return list()
/datum/component/construction/mecha/mbt/get_stockpart_steps()
	return list()
/datum/component/construction/mecha/mbt/get_inner_plating_steps()
	return list()
/datum/component/construction/mecha/mbt/get_outer_plating_steps() // disgusting I know but me stoopid.
	return list()

...
/obj/vehicle/sealed/mecha/spatha/Moved(atom/old_loc, movement_dir, forced, list/old_locs, momentum_change = TRUE)
	. = ..()
	if(has_gravity())
		for(var/mob/living/carbon/human/future_pancake in loc)
			run_over(future_pancake)

/obj/vehicle/sealed/mecha/spatha/proc/run_over(mob/living/carbon/human/crushed)

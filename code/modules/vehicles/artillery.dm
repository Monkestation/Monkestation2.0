...
/obj/vehicle/ridden/artillery
	name = "artillerygun"
	desc = "Whoever just spawned this, your dumb, spawn a SPECIFIC GUN bozo."
	...
	var/spawned_on_undeploy = /obj/machinery/deployable_turret/artillery/light
...
/obj/vehicle/ridden/artillery/wrench_act(mob/living/user, obj/item/wrench/used_wrench)
	. = ..()
	...
	if(!do_after(user, undeploy_time))
		return
	var/obj/undeployed_obj = new spawn_on_undeploy(get_turf(src))
	undeployed_object.modify_max_integrity(max_integrity)
	qdel(src)

/obj/vehicle/ridden/artillery/welder_act(mob/living/user, obj/item/W)
...
/obj/vehicle/ridden/artillery/light
	...
	move_force = MOVE_FORCE_VERY_STRONG
	move_resist = MOVE_FORCE_EXTREMELY_STRONG
	spawned_on_undeploy = /obj/machinery/deployable_turret/artillery/light




	/obj/vehicle/ridden/artillery/light/Initialize(mapload)
	. = ..()
	add_overlay(image(icon, "artillery_light_cover", ABOVE_MOB_LAYER))
	AddElement(/datum/element/ridable, /datum/component/riding/vehicle/artillery/light)

...
/obj/item/assembly/flash/hypnotic/memorizer/AltClick(mob/user)
	...
	var/set_time_setting = tgui_input_number(user, message = "What period of time should the victim forget?", title = "In [time_measure]", default = 2, max_value = 60, min_value = 2)
...
/obj/item/assembly/flash/hypnotic/memorizer/flash_carbon(mob/living/carbon/M, mob/user, confusion_duration = 15, targeted = TRUE, generic_message = FALSE)
	...
	if(targeted)
		if(M.flash_act(1, 1))
			...
			to_chat(M, span_userdanger("A flash of light burns into your eyes! You forget everything in the past [set_time] [time_measure]!"))
			M.adjust_confusion_up_to(10 SECONDS, 20 SECONDS)
			M.apply_status_effect(/datum/status_effect/trance, 200, TRUE)
		else if(user)

...
/datum/fish_trait/proc/remove_from_fish(obj/item/fish/fish)
	...
	if(length(reagents_to_add))
		UnregisterSignal(fish, COMSIG_ATOM_PROCESSED)
...
/datum/fish_trait/toxic/remove_from_fish(obj/item/fish/fish)
	. = ..()
	UnregisterSignal(fish, COMSIG_FISH_EATEN_BY_OTHER_FISH)
	change_venom(fish, reagents_to_add[1], 0.25, 0.1, just_remove = TRUE)
...
/datum/fish_trait/toxic/proc/on_status_change(obj/item/fish/source)
	...
	if(!HAS_TRAIT(source, TRAIT_FISH_STINGER))
		return
	change_venom(source, reagents_to_add[1], 0.25, 0.1)
...
/datum/fish_trait/aggressive/apply_to_fish(obj/item/fish/fish, initial = TRUE)
	. = ..()

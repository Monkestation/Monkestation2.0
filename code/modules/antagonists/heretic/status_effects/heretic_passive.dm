...
/datum/status_effect/heretic_passive/blade/proc/z_impact_react(datum/source, levels, turf/fell_on)
	...
	return NO_Z_IMPACT_DAMAGE /* ZIMPACT_CANCEL_DAMAGE | ZIMPACT_NO_MESSAGE | ZIMPACT_NO_SPIN */
...
/datum/status_effect/heretic_passive/blade/proc/on_shield_reaction(
	...
	if(source.incapacitated(IGNORE_GRAB) /* INCAPACITATED_IGNORING(source, INCAPABLE_GRAB) */)
		return
...
/datum/status_effect/heretic_passive/moon/tick(seconds_between_ticks)
	. = ..()
	var/healing_amount = ((world.time > last_attack + combat_lockout) ? -2 * passive_level * seconds_between_ticks : -4 * passive_level * seconds_between_ticks)
...
/datum/status_effect/heretic_passive/rust/proc/on_move(mob/source, atom/old_loc, dir, forced, list/old_locs)
	...
	if(source.is_touching_rust())
		...
		owner.add_homeostasis_level(id, owner.standard_body_temperature, 5 KELVIN)
	else

	...
	for(var/atom/thing_in_range as anything in range(10, source))
		if(iscarbon(thing_in_range))
			...
			if(IS_HERETIC_OR_MONSTER(close_carbon))
				...
				continue
			if(close_carbon.can_block_magic(MAGIC_RESISTANCE))
				continue
		...
		else if(istype(thing_in_range, /obj/structure/window) || istype(thing_in_range, /obj/structure/grille) || istype(thing_in_range, /obj/structure/window_sill))

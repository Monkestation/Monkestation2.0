...
/obj/machinery/atmospherics/components/binary/pressure_valve/add_context(atom/source, list/context, obj/item/held_item, mob/user)
	...
	context[SCREENTIP_CONTEXT_ALT_LMB] = "Set to maximum recommended target pressure"  //Monkestation edit
...
/obj/machinery/atmospherics/components/binary/pressure_valve/ui_data()
	...
	data["max_pressure"] = round(MAX_PASSIVE_OUTPUT_PRESSURE)  //Monkestation edit
	return data
...
/obj/machinery/atmospherics/components/binary/pressure_valve/ui_act(action, params)
	...
	switch(action)
		...
		if("pressure")
			...
			if(pressure == "max")
				pressure = MAX_PASSIVE_OUTPUT_PRESSURE  //Monkestation edit
				. = TRUE
			else if(text2num(pressure) != null)
				pressure = text2num(pressure)
				. = TRUE
			if(.)
				target_pressure = clamp(pressure, 0, MAX_PASSIVE_OUTPUT_PRESSURE)  //Monkestation edit

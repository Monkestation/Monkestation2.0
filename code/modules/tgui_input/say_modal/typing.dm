...
/datum/tgui_say/proc/start_thinking()
	...
	client.mob.create_thinking_indicator(initial_channel) // monkestation edit: initial_channel
...
/datum/tgui_say/proc/start_typing()
	...
	if(!window_open || !client.typing_indicators || !HAS_TRAIT(client_mob, TRAIT_THINKING_IN_CHARACTER))
		return FALSE
	client_mob.create_typing_indicator(initial_channel) // monkestation edit: initial_channel
...
/datum/tgui_say/proc/stop_typing()
	...
	if(!window_open || !client.typing_indicators || !HAS_TRAIT(client_mob, TRAIT_THINKING_IN_CHARACTER))
		return FALSE
	client_mob.create_thinking_indicator(initial_channel) // monkestation edit: initial_channel

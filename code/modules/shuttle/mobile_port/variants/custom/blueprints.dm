...
/obj/item/shuttle_blueprints/proc/christen(mob/living/user, obj/docking_port/mobile/custom/shuttle, atom/attacked, hand)
	...
	var/has_blueprints = !QDELETED(src) && (src in user.get_all_gear())
	...
	if(!(has_blueprints && is_closed_turf && is_shuttle_turf && off_shuttle && has_bottle && !clumsy_check && !bottle_empty))
		...
		if(!is_closed_turf || clumsy_check)
			...
			if(is_clumsy)
				msg += " What a klutz!"
			var/old_intent_mode = user.istate
			...
			if(hitting_implement && !bottle)
				if(hitting_implement.force)
					playsound(hitting_implement, 'sound/weapons/smash.ogg', hitting_implement.get_clamped_volume(), TRUE, hitting_implement.stealthy_audio ? SILENCED_SOUND_EXTRARANGE : -1, falloff_distance = 0)
				else
					playsound(hitting_implement, 'sound/weapons/tap.ogg', hitting_implement.get_clamped_volume(), TRUE, -1)
			else if(bottle)

...
/obj/machinery/computer/rbmk_console
	...
	layer = OBJ_LAYER
	plane = GAME_PLANE
	/// Reactor currently controlled and monitored by this console.
	var/obj/machinery/rbmk/reactor/linked_reactor
	/// Nearby turbines currently included in operator telemetry.
	var/list/linked_turbines
	/// Maximum automatic turbine-link distance in tiles.
	var/turbine_scan_range = 20
	COOLDOWN_DECLARE(gas_composition_update_cooldown)
	/// Cached coolant composition returned to active UIs.
	var/list/cached_gas_composition
...
/obj/machinery/computer/rbmk_console/Initialize(mapload)
	. = ..()
	alert_radio = new(src)
	alert_radio.keyslot = new /obj/item/encryptionkey/headset_eng
...
/obj/machinery/computer/rbmk_console/proc/auto_link()
	...
	for(var/obj/machinery/rbmk/reactor/reactor in range(RBMK_CONSOLE_SCAN_RANGE, src))
...
/obj/machinery/computer/rbmk_console/ui_data(mob/user)
	...
	data["supermatter_cascade_active"] = !isnull(reactor.supermatter_rod)
	...
	if(reactor.supermatter_rod?.cascade_controller)
		...
		data["supermatter_cascade_final_countdown"] = cascade.final_countdown_started_at > 0
		if(cascade.final_countdown_started_at)
	...
	var/datum/gas_mixture/coolant_mix = reactor.coolant_internal
	...
	for(var/normal_slot_index = 1 to reactor.max_normal_slots)
		...
		rods_list += list(list(
			...
			"active" = normal_rod && !normal_rod.is_depleted(),
	...
	for(var/special_slot_index = 1 to reactor.max_special_slots)
		...
		rods_list += list(list(
			...
			"active" = special_rod && !special_rod.is_depleted(),
...
/obj/machinery/computer/rbmk_console/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	...
	switch(action)
		...
		if("rod_up")
			...
			reactor.control_rod_depth = max(reactor.control_rod_depth - RBMK_CONTROL_ROD_ADJUSTMENT, 0)
		...
		if("rod_down")
			var/old_depth = reactor.control_rod_depth
			reactor.control_rod_depth = min(reactor.control_rod_depth + RBMK_CONTROL_ROD_ADJUSTMENT, RBMK_CONTROL_ROD_MAX)
			if(reactor.control_rod_depth != old_depth)
				playsound(reactor, 'sound/rbmk/switch.ogg', 50, TRUE)
			reactor.update_linked_consoles()
			return TRUE
...
#undef RBMK_CONTROL_ROD_ADJUSTMENT

...
/obj/machinery/computer/rbmk_console/ui_data(mob/user)
	...
	// --- Primary telemetry (rounded) ---
	data["control_rods"]   = round(linked_reactor.control_rod_depth)
	data["temperature"]    = round(linked_reactor.temperature)
	data["instability"]    = round(linked_reactor.instability)
	data["radiation"]      = round(linked_reactor.radiation)
	data["flux"]           = round(linked_reactor.flux)
	data["integrity"]      = round(linked_reactor.reactor_integrity)
	data["max_integrity"]  = linked_reactor.max_reactor_integrity

	// --- History data ---
	data["pressure"] = list()
	for (var/pressureValue in linked_reactor.coolant_pressure_history)
		data["pressure"] += round(pressureValue)

	data["moderator"] = list()
	for (var/modValue in linked_reactor.moderator_history)
		data["moderator"] += round(modValue)
	...
	data["inlet_rate"]  = round(linked_reactor.inlet_rate)
	data["inlet_min"]   = RBMK_INLET_RATE_MIN
	data["inlet_max"]   = RBMK_INLET_RATE_MAX
	data["outlet_target_pressure"] = round(linked_reactor.outlet_target_pressure, 1)
	data["outlet_pressure_max"]    = RBMK_OUTLET_PRESSURE_MAX
	data["pressure_now"]           = round(linked_reactor.pressure, 1)

	/*************************************************************
	 * Gas Composition Snapshot (Rounded)
	 *************************************************************/
	...
	if (linked_reactor.coolant_internal)
		...
		if (total_moles > 0)
			for (var/gas_path in mix.gases)
				...
				var/percent = (moles / total_moles) * 100

...
/obj/machinery/computer/rbmk_console/ui_data(mob/user)
	...
	// Coolant state (pulled from reactor vars)
	data["inlet_open"]  = linked_reactor.inlet_open
	data["outlet_open"] = linked_reactor.outlet_open
	data["flow_rate"]   = linked_reactor.flow_rate
	data["flow_min"]    = linked_reactor.flow_min
	data["flow_max"]    = linked_reactor.flow_max
...
/obj/machinery/computer/rbmk_console/ui_act(action, params)
	...
	switch(action)
		...
		if ("set_flow_rate")
			...
			linked_reactor.set_flow_rate(rate)
			return TRUE

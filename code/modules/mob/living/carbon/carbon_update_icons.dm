...
/mob/living/carbon/proc/update_bandage_overlays()
	...
	for(var/obj/item/bodypart/iter_part as anything in bodyparts)
		var/layer_to_use = BANDAGE_LAYER
		var/render_head = FALSE
		if(istype(iter_part, /obj/item/bodypart/head))
			layer_to_use = UPPER_BANDAGE_LAYER
			render_head = TRUE

		if(isnull(bandage_overlay))
			bandage_overlay = mutable_appearance(layer = -layer_to_use, appearance_flags = KEEP_TOGETHER)
		if(isnull(upper_bandage_overlay))
			upper_bandage_overlay = mutable_appearance(layer = -layer_to_use, appearance_flags = KEEP_TOGETHER)

		if(iter_part.current_gauze)
			var/mutable_appearance/limb_bandage_overlay = iter_part.current_gauze.build_worn_icon(
			default_layer = -layer_to_use, // proc inverts it for us

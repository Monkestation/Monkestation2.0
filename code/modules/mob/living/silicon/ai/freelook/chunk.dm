/**
 * A CHUNK_SIZE x CHUNK_SIZE grid of the map with a list of turfs that can be seen, are visible and are dimmed. \
...
/datum/camerachunk
	...
	/// The network this chunk belongs to.
...
/datum/camerachunk/New(x, y, lower_z, datum/cameranet/net)
	x = GET_CHUNK_COORD(x)
	y = GET_CHUNK_COORD(y)
	...
	for(var/obj/machinery/camera/camera as anything in net.cameras)

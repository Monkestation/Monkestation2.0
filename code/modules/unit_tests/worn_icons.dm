...
/datum/unit_test/worn_icons/Run()
	...
	for(var/obj/item/item_path as anything in (subtypesof(/obj/item) - typesof(/obj/item/mod)))
		...
		if(!cached_slot_flags || (cached_slot_flags & ITEM_SLOT_LPOCKET) || (cached_slot_flags & ITEM_SLOT_RPOCKET) || initial(item_path.item_flags) & ABSTRACT || initial(item_path.item_flags) & TRAIT_NO_WORN_ICON)
			continue

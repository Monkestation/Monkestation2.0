...
/datum/element/digitalcamo/Attach(datum/target)
	...
	RegisterSignal(target, COMSIG_MOVABLE_CAN_TRACK, PROC_REF(track_person))
...
/datum/element/digitalcamo/Detach(datum/target)
	. = ..()
	UnregisterSignal(target, list(COMSIG_ATOM_EXAMINE, COMSIG_MOVABLE_CAN_TRACK))
...
/datum/element/digitalcamo/proc/track_person(datum/source, mob/user)
	SIGNAL_HANDLER

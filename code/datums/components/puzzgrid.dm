...
/proc/create_random_puzzgrid()
	...
	if (isnull(total_lines))
		total_lines = aneri_file_get_line_count(PUZZGRID_CONFIG) // monkestation edit: aneri
	...
	for (var/_ in 1 to PUZZGRID_MAX_ATTEMPTS)
		...
		var/line = aneri_file_seek_line(PUZZGRID_CONFIG, line_number) // monkestation edit: aneri
		if (!line)
			continue

...
//		AssignRole(player, GetJobType(overflow_role), do_eligibility_checks = FALSE) //monkestation removal
	...
	for(var/level in level_order)
		...
		for(var/mob/dead/new_player/player in unassigned)
			...
			if(!(get_player_details(player)?.patreon?.is_donator() || is_admin(player.client)) && PopcapReached())
				RejectPlayer(player)
...
/datum/controller/subsystem/job/proc/check_job_eligibility(mob/dead/new_player/player, datum/job/possible_job, debug_prefix = "", add_job_to_log = FALSE)
	...
	if(((possible_job in holiday_restricted) || !isnull(possible_job.job_req_donor)) && (!is_admin(player.client)))

...
/proc/load_icon_exists_cache()
	...
	if(!fexists("icon_exists_cache.json"))
		log_icon_exists_cache("icon_exists_cache.json doesn't exist, not loading cache")
		return
	var/cache_file = rustg_file_read("icon_exists_cache.json")
	if(!rustg_json_is_valid(cache_file))
		log_icon_exists_cache("did not load icon_exists cache: file exists but wasn't valid json")
	...
	if(!islist(icons))
		log_icon_exists_cache("did not load icon_exists cache: 'icons' key was not a list")
	...
	if(!isnull(cache_revision))
		...
		if(cache_revision != revision)
			log_icon_exists_cache("did not load icon_exists cache: revision ([cache_revision]) mismatched current server revision ([revision])")
			return
		log_icon_exists_cache("loaded icon_exists cache for [revision]")
	else
		log_icon_exists_cache("loaded icon_exists cache without verifying revision")
	return icons
#endif

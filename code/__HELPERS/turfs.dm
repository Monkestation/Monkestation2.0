...
/proc/get_safe_station_turfs(list/areas_to_pick_from = GLOB.the_station_areas, care_about_dense_objects = FALSE, allow_outdoor_areas = FALSE) as /list
	RETURN_TYPE(/list)
	. = list()
	for(var/area/turf_area as anything in areas_to_pick_from)
		...
		for(var/turf/checked_turf in turf_list)
			if(!checked_turf.density && (turf_area::area_flags & VALID_TERRITORY) && !isgroundlessturf(checked_turf) && (allow_outdoor_areas || !turf_area.outdoors))
				var/clear = TRUE

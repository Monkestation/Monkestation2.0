...
/proc/get_hearers_in_view(view_radius, atom/source)
	...
	//so by representing every hearing contents on a turf with a single /mob/abstract/oranges_ear containing references to all of them, we are:
	...
	//2. typechecking all mobs in the output to only actually return mobs of type /mob/abstract/oranges_ear
	...
	for(var/mob/abstract/oranges_ear/ear in hearers(view_radius, center_turf))
		. += ear.references

...
#define DM_32BIT
	...
	defined(_M_X64) || defined(_WIN64) || defined(WIN64)
#define _X64
#define _X86ORX64
#define DM_64BIT
...
#define DUNGPUB
#define BYOND_EXPORT                                                                                                   \
	__attribute__((visibility("default"))) // for functions in user-defined .so libraries for use with call_ext()
#endif
...
/**
 * Creates a string and sets CByondValue to a reference to that string, and increases the reference count. See REFERENCE
 * COUNTING in byondapi.h. Blocks if not on the main thread. If string creation fails, the struct is set to null.
 * @param v Pointer to CByondValue
...
/**
 * Fills a CByondValue struct with a reference to a string with a given ID. Does not validate, and does not increase the
 * reference count. If the strid is NONE, it will be changed to 0.
 * @param v Pointer to CByondValue
 ...
 * callback runs on the main thread. Blocking is optional. If already on the main thread, the block parameter is
 * meaningless.
 ...
 */
DUNGPUB CByondValue Byond_ThreadSync(ByondCallback callback, void* data, bool block);
 ...
 */
DUNGPUB u4c
Byond_GetStrId(char const* str); // does not add a string to the tree if not found; returns NONE if no string match
/**
...
/**
 * Reads items as key,value pairs from an associative list, storing them sequentially as key1, value1, key2, value2,
 * etc. Blocks if not on the main thread.
 * @param loc The list to read
 * @param list CByondValue array, allocated by caller (can be null if querying length)
 * @param len Pointer to length of array (in items); receives the number of items read on success, or required length of
 * array if not big enough
 ...
 * @return True on success
 */
DUNGPUB bool Byond_CallProc(CByondValue const* src, char const* name, CByondValue const* arg, u4c arg_count,
							CByondValue* result);
/**
 ...
 * @see Byond_GetStrId()
 */
DUNGPUB bool Byond_CallProcByStrId(CByondValue const* src, u4c name, CByondValue const* arg, u4c arg_count,
								   CByondValue* result);
 ...
 * @return True on success
 */
DUNGPUB bool Byond_CallGlobalProc(char const* name, CByondValue const* arg, u4c arg_count,
								  CByondValue* result); // result MUST be initialized first!
/**
 ...
 * @see Byond_GetStrId()
 */
DUNGPUB bool Byond_CallGlobalProcByStrId(u4c name, CByondValue const* arg, u4c arg_count,
										 CByondValue* result); // result MUST be initialized first!
 ...
 * @param buflen Pointer to length of array in bytes; receives the string length (including trailing null) on success,
 * or required length of array if not big enough
 * @return True on success; false with *buflen=0 for failure; false with *buflen=required size if array is not big
 * enough
 */
 ...
 * @return True on success
 */
DUNGPUB bool Byond_NewArglist(CByondValue const* type, CByondValue const* arglist,
							  CByondValue* result); // result MUST be initialized first!
 ...
 * @return True on success
 */
DUNGPUB bool Byond_XYZ(CByondValue const* src,
					   CByondXYZ* xyz); // still returns true if the atom is off-map, but xyz will be 0,0,0
 ...
 * @return True on success
 */
DUNGPUB bool Byond_PixLoc(CByondValue const* src,
						  CByondPixLoc* pixloc); // still returns true if the atom is off-map, but pixloc will be 0,0,0
 ...
 * @return True on success
 */
DUNGPUB bool Byond_BoundPixLoc(
	CByondValue const* src, u1c dir,
	CByondPixLoc* pixloc); // still returns true if the atom is off-map, but pixloc will be 0,0,0
 ...
 * already persistent. Blocks if not on the main thread.
 ...
 * persistent. This cannot be used for temporary references. See ByondValue_DecTempRef() for those. Blocks if not on the
 * main thread.
 ...
 * little faster. Only works on the main thread. Does nothing on other threads.
 * @param src The object to decref
 */

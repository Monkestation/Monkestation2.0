...
export class AnimatedNumber extends Component<AnimatedNumberProps, {}> {
  ...
  shouldComponentUpdate(newProps: AnimatedNumberProps) {
    ...
    // We render the inner `span` directly using a ref to bypass react diffing
    // and reach 60 frames per second--tell react not to re-render this tree.
    return false;
  }
  ...
  tick() {
    ...
    if (this.ref.current) {
      // Directly update the inner span, without bothering react.
